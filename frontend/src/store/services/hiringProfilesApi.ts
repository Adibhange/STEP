...
export const hiringProfilesApi = stepApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    >({
      query: (body) => ({
        url: `/role-tier-matrix/bulk-assign`,
        method: 'POST',
        body,
      }),
    ...
    >({
      query: (body) => ({
        url: body.id ? `/hiring-blueprints/${body.id}` : `/hiring-blueprints`,
