	...
	endpoints: (builder) => ({
		...
		>({
			query: ({ candidateId, flowTemplateId, vacancyPipelineFlowId }) => ({
				...
				method: "POST",
				body: {
					vacancyPipelineFlowId: vacancyPipelineFlowId || flowTemplateId,
				},
			}),
			invalidatesTags: ["Candidates"],
		}),
		...
		deleteCandidateDocument: builder.mutation<
			ApiEnvelope<any>,
			{ candidateId: number; documentId: number }
		>({
		...
		registerUniversalCandidate: builder.mutation<
			ApiEnvelope<any>,
			Record<string, any>
		>({
			query: (data) => ({
