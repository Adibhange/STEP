...
export const CandidateBulkFlowAssignment: React.FC<
	CandidateBulkFlowAssignmentProps
> = ({ vacancyId, vacancyTitle }) => {
	const numVacId =
		vacancyId ? parseInt(String(vacancyId).replace(/\D/g, ""), 10) : undefined;
	const { data: candidatesRes, isLoading } = useGetCandidatesQuery(
		numVacId ? { vacancyId: numVacId } : undefined,
	);
	const { data: vacancyRes } = useGetVacancyByIdQuery(numVacId!, {
		skip: !numVacId,
	});
	const [assignPipelineFlow, { isLoading: isAssigning }] =
		useAssignPipelineFlowMutation();
	...
	const flowOptions: FlowOption[] = useMemo(
		() =>
			(vacancyRes?.data?.pipelineFlows || []).map((f: any) => ({
				id: f.id,
	...
	const rawCandidates = useMemo(() => {
		...
		if (vacancyId) {
			list = list.filter((c: any) => {
				return (
					c.vacancyId !== null &&
					c.vacancyId !== undefined &&
					(String(c.vacancyId) === String(vacancyId) ||
						String(c.vacancyId) === String(numVacId))
				);
			});
		}
		return list;
	...
	const candidates: (CandidateBulkItem & {
		hasPipelineFlowAssigned: boolean;
	})[] = useMemo(() => {
	...
	const unassignedCandidates = useMemo(
		() => candidates.filter((c) => !c.hasPipelineFlowAssigned),
		[candidates],
	);

	const persistAssignments = async (
		plan: { candidateId: number; vacancyPipelineFlowId: number }[],
		label: string,
	) => {
		...
		if (failed === 0) {
			toast.success(`${label} Applied`, {
				description: `${succeeded} candidate(s) assigned to their pipeline flow.`,
			});
		} else {
			toast.error(`${label} Partially Applied`, {
				description: `${succeeded} succeeded, ${failed} failed (likely already assigned).`,
			});
		}
	};
	...
	const handleApply5050Split = () => {
		...
		const plan = unassignedCandidates.map((c, idx) => ({
			...
			vacancyPipelineFlowId:
				flowOptions[idx % Math.min(2, flowOptions.length)].id,
		}));
	...
	const handleSingleCandidateFlowChange = (
		candidateId: string,
		flowId: number,
	) => {
		persistAssignments(
			[{ candidateId: Number(candidateId), vacancyPipelineFlowId: flowId }],
			"Pipeline Flow Assignment",
		);
	};
		...
		<div className='flex flex-col gap-4'>
			{flowOptions.length === 0 && (
				<div className='p-3 rounded-lg bg-amber-50 border border-amber-200 text-[12px] text-amber-700 font-semibold flex items-center gap-2'>
					...
					/>
					<span>
						No pipeline flow version has been configured for this vacancy yet —
						add one in the Flow Versions tab before assigning candidates.
					</span>
				</div>
			)}
					...
					<motion.tbody
						...
						{isLoading && (
							<tr>
								<td
									colSpan={7}
									className='py-8 text-center text-xs text-[var(--text-tertiary)] font-mono'>
									...
									<motion.tr
										...
										<td className='py-3 px-4'>
											...
											:	<CustomSelect
													...
													onChange={(val) =>
														val &&
														handleSingleCandidateFlowChange(c.id, Number(val))
													}
													options={flowOptions.map((fv) => ({
														value: String(fv.id),
														label: fv.versionName,
													}))}
													widthClass='w-72'
												/>
											}
