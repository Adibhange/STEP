...
export const VacancyCandidatesTab: React.FC<VacancyCandidatesTabProps> = ({
	vacancy,
}) => {
	...
	const {
		...
		refetch,
	} = useGetCandidatesQuery(numVacId ? { vacancyId: numVacId } : undefined);

	// Local overrides for optimistic UI updates (e.g. Authorize Tech)
	const [localOverrides, setLocalOverrides] = useState<
		Record<number, Partial<CandidateRow>>
	>({});
	...
	const candidates: CandidateRow[] = useMemo(() => {
		...
		return list.map((c: any) => {
			...
			let currentRoundStatus: "Passed" | "Failed" | "In-Progress" =
				"In-Progress";
			if (
				c.status === "Rejected" ||
				c.status?.includes("Eliminated") ||
				(aptScore !== null && aptScore < 70)
			) {
			...
			const baseRow: CandidateRow = {
				...
				sourceChannel:
					c.sourceChannel || (isDirectHiring ? "Direct Sourced" : "Walk-in QR"),
	...
	const [statusFilter, setStatusFilter] = useState<
		"ALL" | "AWAITING_AUTH" | "IN_TEST" | "INTERVIEW" | "OFFERED" | "ELIMINATED"
	>("ALL");
	const [sortBy, setSortBy] = useState<"time" | "score" | "exp" | "name">(
		"time",
	);
	...
	const filterCounts = useMemo(
		() => ({
			...
			awaitingAuth: candidates.filter((c) => c.status === "Awaiting Tech Auth")
				.length,
			inTest: candidates.filter(
				(c) =>
					c.currentRoundStatus === "In-Progress" || c.status.includes("Taking"),
			).length,
			interview: candidates.filter((c) => c.status.includes("Interview"))
				.length,
			offered: candidates.filter((c) => c.status === "Offered").length,
			eliminated: candidates.filter(
				(c) =>
					c.status.includes("Eliminated") || c.currentRoundStatus === "Failed",
			).length,
		}),
		[candidates],
	);
	...
	const handleToggleSelectAll = () => {
		if (
			selectedIds.size === processedCandidates.length &&
	...
	const handleBulkAuthorize = () => {
		...
		if (count === 0) {
			toast.error("No Eligible Candidates", {
				description:
					"Selected candidates have either not passed Aptitude (Cutoff: 70%) or are already authorized.",
			});
			return;
		}
	...
	const handleLiveSync = async () => {
		...
		try {
			...
			toast.success("Live Sync Complete", {
				description: "Roster synced with live assessment engine.",
			});
		} catch {
			toast.error("Sync Failed", {
				description: "Could not refresh candidate roster.",
			});
		} finally {
			setIsSyncing(false);
		}
	...
	const handleExportExcel = async () => {
		try {
			...
			toast.error("Export Failed", {
				description:
					err?.message || "Failed to export candidate records to Excel.",
			});
		}
	};
	...
	const dashboardCandidateModalData: DashboardCandidate | null = useMemo(() => {
		...
		return {
			...
			source: "WalkIn",
			stage:
				selectedCandidate.status.includes("Interview") ? "Technical"
				: selectedCandidate.status === "Offered" ? "Offered"
				: "Screening",
			currentRound: selectedCandidate.currentRoundName,
			assignedInterviewer: "Evaluator Panel",
			status:
				selectedCandidate.status.includes("Eliminated") ? "Rejected"
				: selectedCandidate.status === "Offered" ? "Offered"
				: "Screening",
	...
	const filterTabs: {
		...
		{
			id: "AWAITING_AUTH",
			label: "⚡ Awaiting Tech Auth",
			count: filterCounts.awaitingAuth,
			badgeStyle: "bg-[var(--accent-indigo)] text-white font-bold",
		},
		{ id: "IN_TEST", label: "In Assessment", count: filterCounts.inTest },
		{
			id: "INTERVIEW",
			label: "Interview Stage",
			count: filterCounts.interview,
		},
							...
							<button
								...
								<span
									className={`text-[10px] px-1.5 py-0.5 rounded-full font-mono font-bold ${
										tab.badgeStyle ? tab.badgeStyle
										: isActive ? "bg-[var(--accent-indigo)] text-white"
						...
						<tbody className='divide-y divide-[var(--border-soft)]'>
							{paginatedCandidates.length === 0 ?
								<tr>
									<td
										colSpan={7}
										className='py-14 text-center text-[var(--text-tertiary)] text-xs'>
										...
										<tr
											...
											<td className='py-2.5 px-2'>
												...
												<div className='text-[11px] text-[var(--text-tertiary)] flex items-center gap-1.5 mt-0.5 whitespace-nowrap'>
													...
													<span className='text-[var(--text-tertiary)]/50'>
														•
													</span>
													...
													<div className='mt-0.5'>
														{(
															...
															<span className='inline-flex items-center gap-1 text-[11px] font-mono font-bold text-rose-400'>
																<span>
																	Aptitude:{" "}
																	{c.aptitudeScore ?? c.currentRoundScore}%
																</span>
				...
				{selectedIds.size > 0 && (
					<motion.div
						...
						<button
							...
							/>
							<span>
								Authorize Technical Round ({eligibleSelectedCandidates.length})
							</span>
						</button>
