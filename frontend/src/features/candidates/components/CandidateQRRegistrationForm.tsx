...
export const CandidateQRRegistrationForm: React.FC<
	CandidateQRRegistrationFormProps
> = ({ driveInfo }) => {
	const [registerCandidate, { isLoading }] =
		useRegisterCandidateViaQRMutation();
	...
	const { data: eligibilityRes } = useCheckQRRegistrationEligibilityQuery(
		{
			code: driveInfo.code,
			email: emailValid ? debouncedEmail : undefined,
			phone: phoneValid ? debouncedPhone : undefined,
		},
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!isFresher) {
			...
			if (isNaN(expNum) || expNum < 0) {
				newErrors.totalExperienceYears =
					"Please enter a valid experience (years).";
			}
		}
		...
		try {
			const payload = {
				...
				totalExperienceYears:
					isFresher ? 0 : parseFloat(totalExperienceYears) || 0,
				currentCTC:
					!isFresher && currentCTC ? parseFloat(currentCTC) : undefined,
				expectedCTC: expectedCTC ? parseFloat(expectedCTC) : undefined,
				noticePeriodDays:
					!isFresher && noticePeriodDays ?
			...
			<div className='w-full max-w-md mx-auto bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[var(--radius-2xl)] shadow-xl overflow-hidden p-6 text-center animate-in fade-in zoom-in duration-300'>
					...
					<div className='mt-3 pt-3 border-t border-[var(--border-soft)] grid grid-cols-2 gap-2 text-xs'>
						<div>
							...
							<span className='font-bold text-[var(--text-primary)] truncate block'>
								{registeredCandidate.vacancyTitle ||
									driveInfo.vacancyTitle ||
									"Software Engineer"}
							</span>
						</div>
						<div>
				...
				<div className='p-3.5 rounded-[var(--radius-lg)] bg-[var(--surface-2)] border border-[var(--border-soft)] text-left text-xs text-[var(--text-secondary)] space-y-1.5 mb-6'>
					...
					<ul className='list-disc pl-4 space-y-1 text-[11.5px] text-[var(--text-tertiary)]'>
						<li>
							Show this Registration Token to the coordinators at the entrance.
						</li>
						<li>
							You will receive your computer node assignment for Round 1
							assessment.
						</li>
					</ul>
				</div>
					...
					<div className='space-y-3.5 animate-in fade-in duration-200'>
						...
						{isBlockedByCooldown && (
							<div className='p-3 rounded-[var(--radius-xl)] bg-[var(--status-danger-bg)] border border-[var(--status-danger-border)] text-[var(--status-danger-text)] text-xs flex items-start gap-2'>
								...
								/>
								<span>
									You already applied for this role on{" "}
									{formatDate(eligibility!.lastAppliedAt)}. You can re-apply for
									the same role after {formatDate(eligibility!.eligibleFrom)}{" "}
									(90-day cooldown).
								</span>
							</div>
						)}
