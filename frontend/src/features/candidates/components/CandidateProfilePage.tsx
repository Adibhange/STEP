...
export const CandidateProfileSkeleton: React.FC = () => (
	<div className='flex flex-col gap-4 pb-6 p-3.5 sm:p-5 bg-[var(--canvas)] min-h-screen text-[13px] font-sans relative animate-in fade-in duration-150'>
			...
			<div className='lg:col-span-4 flex flex-col gap-3.5'>
				...
				<div className='bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[var(--radius-xl)] p-4 shadow-[var(--shadow-xs)] flex flex-col gap-3.5 relative overflow-hidden'>
					...
					{[1, 2, 3, 4].map((i) => (
						<div
							key={i}
							className='flex flex-col gap-2 pt-2 border-t border-[var(--border-soft)]'>
							<Skeleton
								width={110}
								height={14}
			...
			<div className='lg:col-span-8 flex flex-col gap-4'>
				...
				<div className='flex flex-col gap-3 pl-4 border-l-2 border-[var(--border-soft)] ml-3'>
					{[1, 2, 3, 4, 5].map((i) => (
						<div
							key={i}
							className='flex flex-col gap-2 p-3.5 rounded-xl border border-[var(--border-default)] bg-[var(--surface-2)]'>
...
export const CandidateProfilePage: React.FC<CandidateProfilePageProps> = ({
	...
	} = useGetCandidateByIdQuery(numericId);
	const { data: candidatesListRes, isLoading: isListLoading } =
		useGetCandidatesQuery();
	...
	const [selectedDocPreview, setSelectedDocPreview] =
		useState<CandidateDocument | null>(null);
	const [showEvaluationModal, setShowEvaluationModal] = useState(false);
	const [selectedEvaluationSessionId, setSelectedEvaluationSessionId] =
	...
	const assignedFlowVersionName = useMemo(() => {
		if (typeof window !== "undefined") {
			const saved =
				localStorage.getItem(`step_candidate_flow_${numericId}`) ||
				localStorage.getItem(`step_candidate_flow_${candidateId}`);
	...
	useEffect(() => {
		...
		if (apiData) {
			const isFresher =
				apiData.totalExperienceYears === 0 || apiData.experienceYears === 0;
			const history =
				apiData.pipelineProgress || apiData.pipelineProgressHistory;
			...
			const r1Failed =
				r1?.status?.toLowerCase() === "failed" ||
				r1?.status?.toLowerCase() === "rejected";
			const r2Failed =
				r2?.status?.toLowerCase() === "failed" ||
				r2?.status?.toLowerCase() === "rejected";
			const r3Failed =
				r3?.status?.toLowerCase() === "failed" ||
				r3?.status?.toLowerCase() === "rejected";

			const isTrulyRejected = r1Failed || (r2Failed && r3Failed);
			const candidateStatus =
				isTrulyRejected ? "Rejected"
				: apiData.status === "Offered" || apiData.status === "Hired" ?
					apiData.status
				:	"In-Progress";

			const photoDoc = apiData.documents?.find(
				(d: any) =>
					d.documentType === "Profile Photo" || d.documentType === "Avatar",
			);
			const photoUrl =
				photoDoc ?
					`${getApiBaseUrl()}/candidates/${numericId}/documents/${photoDoc.id}/file`
				:	"";
			const savedAvatar =
				photoUrl ||
				(typeof window !== "undefined" ?
					localStorage.getItem(`step_candidate_avatar_${numericId}`)
				:	null) ||
				apiData.avatarUrl ||
				"";
			...
			const cleanRole =
				(
					apiData.vacancyTitle &&
					apiData.vacancyTitle !== "Walk-In Registration"
				) ?
					apiData.vacancyTitle
						.replace(/[-–—]\s*⚡?\s*1-Click Drive/gi, "")
						.trim()
				: apiData.role && apiData.role !== "Applicant" ? apiData.role
				: "Senior .NET Architect";

			setCandidate({
				id: String(apiData.id || candidateId),
				code:
					apiData.candidateCode ||
					apiData.code ||
					`CND-2026-${apiData.id || candidateId}`,
				name:
				...
				appliedDate:
					apiData.createdAt ?
						new Date(apiData.createdAt).toLocaleDateString("en-GB", {
							day: "2-digit",
							month: "short",
							year: "numeric",
						})
					:	"18 Aug 2026",
				experience:
					isFresher ? "Fresher (0 Yrs)" : (
						`${apiData.totalExperienceYears ?? apiData.experienceYears ?? 0} Years`
					),
				...
				source:
					(
						(apiData.registrationChannel || "")
							...
							.includes("direct") ||
						(history || []).some((p: any) =>
							(p.roundTitle || "").toLowerCase().includes("auto-passed"),
						)
					) ?
						"Direct Sourced"
					:	apiData.registrationChannel || "Walk-in Scan",
				refType:
					apiData.refType ||
					(apiData.referralEmployeeName ? "Internal" : "Direct"),
			...
			if (history && Array.isArray(history) && history.length > 0) {
				...
				const liveStages: StageItem[] = history.map((p: any) => {
					...
					const hasCompletedTest = Boolean(
						p.completedAt ||
						p.scoreObtained !== null ||
						p.candidateExamSessionId,
					);
					const isDirectorRound =
						p.roundType === "Director" ||
						(p.roundTitle || "").toLowerCase().includes("director");
					const isOfferRound =
						(p.roundTitle || "").toLowerCase().includes("offer") ||
						(p.roundType === "Director" && p.roundNumber === history.length);

					const cleanRoundName = (() => {
						const raw = p.roundTitle || `Round ${p.roundNumber}`;
						if (
							p.roundNumber === 2 &&
							(raw.toLowerCase().includes("mcq only") ||
								raw.toLowerCase().includes("standard assessment track"))
						) {
							if (
								cleanRole.toLowerCase().includes("sql") ||
								cleanRole.toLowerCase().includes("database")
							) {
					...
					return {
						...
						feedback:
							...
							:	(() => {
									...
									const scoreStr =
										hasScore ? `Candidate Exam Score: ${p.scoreObtained}%` : "";
									...
									if (isCurrentRoundFailed) {
										return (
											p.remarks || "Candidate failed evaluation for this round."
										);
									}
									if (p.remarks) {
										return p.remarks;
				...
				const isApplied =
					apiData.status === "Applied" ||
					apiData.currentStage === "Applied" ||
					apiData.currentStage === "Registered";
				const isWalkInDrive =
					apiData.registrationChannel === "Walk-in Scan" ||
					apiData.registrationChannel === "WalkIn" ||
					(candidate.source && candidate.source.toLowerCase().includes("walk"));
				...
				if (isWalkInDrive) {
					...
					if (
						assignedFlowVersionName.includes("Fast-Track Technical") ||
						assignedFlowVersionName.includes("Flow Version 2")
					) {
				...
				setStagesData([
					{
						...
						status:
							!isWalkInDrive ? "Passed"
							: isApplied ? "In Progress"
							: "Passed",
						statusType:
							!isWalkInDrive ? "passed"
							: isApplied ? "pending"
						...
						date: candDate,
						interviewer:
							!isWalkInDrive ? "Talent Acquisition" : "Assigned Evaluator",
					...
					},
					{
						...
						feedback:
							isTrulyRejected ?
								"Candidate failed Round 2 evaluation."
							:	"Stage 2 evaluation pending.",
					...
					},
					{
						...
						mode: "In Office",
						feedback:
							"Stage 3 evaluation pending (Technical F2F & Live Coding open).",
						result: "Pending",
			...
			if (
				...
				const liveDocs: CandidateDocument[] = apiData.documents.map(
					(d: any, idx: number) => ({
						id: d.id || idx + 1,
	...
	const [profileValidationToast, setProfileValidationToast] = useState<
		string | null
	>(null);
	...
	const { data: offerRes } = useGetOfferByIdQuery(offerLetterId ?? 0, {
		skip: !offerLetterId,
	});
	const [generateOfferLetter, { isLoading: isGeneratingOffer }] =
		useGenerateOfferLetterMutation();
	const [approveOffer, { isLoading: isApprovingOffer }] =
		useApproveOfferMutation();
	...
	const [selectedFeedbackStage, setSelectedFeedbackStage] =
		useState<StageItem | null>(null);
	...
	const [scorecardRecommendation, setScorecardRecommendation] = useState<
		"Pass" | "Fail" | "Hire" | "Reject" | "OnHold"
	>("Pass");

	const currentUserId =
		typeof window !== "undefined" ?
			Number(localStorage.getItem("step_user_id")) || null
		:	null;
	const [submitInterviewFeedback, { isLoading: isSubmittingFeedback }] =
		useSubmitInterviewFeedbackMutation();
	const [publishInterviewResult, { isLoading: isPublishingDecision }] =
		usePublishInterviewResultMutation();
	const [evaluateCandidateStage] = useEvaluateCandidateStageMutation();
	const { data: feedbackInterviewRes } = useGetInterviewByIdQuery(
		selectedFeedbackStage?.interviewId ?? 0,
		{
			skip:
	...
	const myExistingScorecard = feedbackInterviewRes?.data?.roundDetails.find(
		(d) => d.panelistUserId === currentUserId,
	);
	...
	useEffect(() => {
		...
		setScorecardRecommendation(
			myExistingScorecard.recommendation as "Hire" | "Reject" | "OnHold",
		);
		setFeedbackText(myExistingScorecard.comments || "");
	}, [myExistingScorecard]);

	// Assign Interviewer Modal State
	const [selectedAssignStage, setSelectedAssignStage] =
		useState<StageItem | null>(null);
		...
		() =>
			(usersRes?.data || [])
				...
				.map((u) => ({
					value: String(u.id),
					label:
						`${u.firstName || ""} ${u.lastName || ""}`.trim() + ` (Director)`,
				})),
		[usersRes],
	);
	...
	useEffect(() => {
		if (selectedAssignStage) {
			const options =
				selectedAssignStage.isDirectorRound ? directorOptions : (
					interviewerOptions
				);
			if (options.length > 0) {
				const currentlyAssigned = options.find(
					(o) => Number(o.value) === selectedAssignStage.interviewerUserId,
				);
				setAssignedInterviewer(
					currentlyAssigned ? currentlyAssigned.value : options[0].value,
				);
			}
		}
	...
	const handleSaveProfileEdit = async (
		...
		if (!f.email || !emailRegex.test(f.email.trim())) {
			return fireValidationToast(
				"Please enter a valid email address (e.g. name@domain.com).",
			);
		}

		// Phone — must be 10 digits (ignoring spaces, dashes, +91 prefix)
		const digitsOnly = (f.phone || "")
			.replace(/[\s\-+]/g, "")
			.replace(/^91/, "");
		if (!digitsOnly || !/^\d{10}$/.test(digitsOnly)) {
			return fireValidationToast(
				"Mobile number must be exactly 10 digits (e.g. 98765 43210).",
			);
		}
		...
		if (isNaN(expNum) || expNum < 0 || expNum > 60) {
			return fireValidationToast(
				"Experience must be a valid number between 0 and 60 years.",
			);
		}
		...
		if (!f.currentCtc || !ctcRegex.test(f.currentCtc)) {
			return fireValidationToast(
				"Current CTC must include a numeric value (e.g. ₹ 9.5 LPA).",
			);
		}
		if (!f.expectedCtc || !ctcRegex.test(f.expectedCtc)) {
			return fireValidationToast(
				"Expected CTC must include a numeric value (e.g. ₹ 12 LPA).",
			);
		}
		...
		try {
			...
			const numericExp =
				parseFloat((f.experience || "").replace(/[^\d.]/g, "")) || 0;
			const numericCurrentCtc =
				parseFloat((f.currentCtc || "").replace(/[^\d.]/g, "")) || 0;
			const numericExpectedCtc =
				parseFloat((f.expectedCtc || "").replace(/[^\d.]/g, "")) || 0;
			const numericNoticeDays =
				parseInt((f.noticePeriod || "").replace(/\D/g, ""), 10) || 30;
			const numericPassingYear =
				parseInt((f.passingYear || "").replace(/\D/g, ""), 10) || 2026;
			const numericMarks =
				parseFloat((f.percentage || "").replace(/[^\d.]/g, "")) || 85;
			...
			toast.success("Profile Saved", {
				description: "Candidate profile details updated successfully",
			});
		} catch (err: any) {
	...
	const handleDeleteDocument = async (docId: number, docName: string) => {
		try {
			await deleteCandidateDocApi({
				candidateId: numericId,
				documentId: docId,
			}).unwrap();
			setDocumentsData((prev) => prev.filter((d) => d.id !== docId));
			toast.info("Document Deleted", {
				description: `"${docName}" removed from candidate profile.`,
			});
		} catch (err: any) {
			toast.error("Delete Failed", {
				description: err.data?.message || "Failed to delete document.",
			});
		}
	};
	...
	const handleDownloadDocument = (docName: string, docId?: number) => {
		...
		if (targetId) {
			window.open(
				`${getApiBaseUrl()}/candidates/${numericId}/documents/${targetId}/download`,
				"_blank",
			);
	...
	const handleRolloutOffer = async (
		e: React.SyntheticEvent<HTMLFormElement>,
	) => {
		e.preventDefault();
		...
		if (!offerLetterId) {
			...
			if (!offerCtc || Number.isNaN(ctcNum) || ctcNum <= 0) {
				toast.error("Invalid CTC", {
					description: "Enter a valid offered CTC greater than 0.",
				});
				return;
			}
			if (!offerJoiningDate) {
				toast.error("Joining Date Required", {
					description: "Select a joining date.",
				});
				return;
			}
			...
			try {
				...
				toast.success("Offer Letter Generated", {
					description:
						"PDF generated and saved — awaiting Director PIN approval before it's final.",
				});
			} catch (err) {
				const description =
					(err as { data?: { errors?: string[]; message?: string } })?.data
						?.errors?.[0] ||
		...
		if (currentOfferStatus === "PendingApproval") {
			if (offerApprovalPin.length !== 4) {
				toast.error("Invalid PIN", {
					description: "Enter the Director's 4-digit security PIN.",
				});
				return;
			}
			try {
				await approveOffer({
					id: offerLetterId,
					directorPin: offerApprovalPin,
				}).unwrap();
				setOfferApprovalPin("");
				toast.success("Offer Approved", {
					description:
						"Director approval recorded — the offer letter is now finalized.",
				});
			} catch (err) {
				const description =
					(err as { data?: { errors?: string[]; message?: string } })?.data
	...
	const handleDownloadOffer = async () => {
		...
		try {
			const token =
				typeof window !== "undefined" ?
					localStorage.getItem("step_token")
				:	null;
			const res = await fetch(
				`${getApiBaseUrl()}/offers/${offerLetterId}/download`,
				{
			...
			toast.error("Download Failed", {
				description: "Could not download the offer letter PDF.",
			});
		}
	};
	...
	const handleRetakeAptitude = async (roundId: number) => {
		try {
			await evaluateCandidateStage({
				...
				isRetake: true,
				remarks:
					"Candidate granted retake for Round 1 Aptitude (Attempt 2 of 2).",
			}).unwrap();
			setStagesData((prev) =>
				prev.map((s) =>
					s.id === roundId ?
						{
							...
							score: null,
							feedback:
								"Aptitude re-test authorized (Attempt 2 of 2). Awaiting candidate exam submission.",
						}
					:	s,
				),
			);
			toast.success("Aptitude Retake Granted", {
				description:
					"Candidate can now re-attempt the Aptitude assessment (Attempt 2 of 2).",
			});
		} catch (err: any) {
			const errorMsg =
				err?.data?.message ||
				err?.data?.errors?.[0] ||
				"Could not grant aptitude retake.";
	...
	const handleSaveFeedback = async (
		...
		if (selectedFeedbackStage.roundType === "Assessment") {
			try {
				...
				toast.error("Submission Failed", {
					description: "Could not submit assessment evaluation.",
				});
			}
			return;
		}
			...
			try {
				if (!targetInterviewId) {
					...
					const schedRes = await scheduleInterview({
						candidateId: numericId,
						interviewerUserId:
							currentUser?.id ? Number(currentUser.id) : currentUserId || 1,
				...
				const mappedRecommendation: "Hire" | "Reject" | "OnHold" =
					(
						scorecardRecommendation === "Fail" ||
						scorecardRecommendation === "Reject"
					) ?
						"Reject"
					: scorecardRecommendation === "OnHold" ? "OnHold"
					: "Hire";
				...
				await publishInterviewResult({
					...
					passed: isPassed,
					remarks:
						feedbackText || `Scorecard submitted (${mappedRecommendation}).`,
				}).unwrap();
				...
				const description =
					(err as { data?: { message?: string; errors?: string[] } })?.data
						?.errors?.[0] ||
					(err as { data?: { message?: string } })?.data?.message ||
		...
		if (directorDecision === "hold") {
			...
			toast.success("Decision Deferred", {
				description: "Candidate placed On Hold — status unchanged.",
			});
			return;
		}
		...
		if (directorPin.length !== 4) {
			toast.error("Invalid PIN", {
				description:
					"Enter the Director's 4-digit security PIN to authorize this hiring decision.",
			});
			return;
		}
		...
		try {
			...
			await evaluateCandidateStage({
				...
				remarks:
					feedbackText ||
					(isHired ?
						"Director approved — Candidate Hired."
					:	"Director decision — Candidate Rejected."),
			...
			setCandidate((prev) => ({
				...prev,
				status: isHired ? "Hired" : "Rejected",
			}));
			refetchCandidate();
			toast.success(isHired ? "Candidate Hired" : "Decision Recorded", {
				description:
					isHired ?
						`Director PIN verified — ${candidate.name} is successfully marked as Hired.`
		...
		if (isAssessment && !selectedAssignStage.isDirectorRound) {
			try {
				...
				toast.error("Failed to Assign Evaluator", {
					description:
						err?.data?.message ||
						"Could not save evaluator assignment to database.",
				});
			}
			return;
		}
		...
		try {
			...
			try {
				if (/^\d{4}-\d{2}-\d{2}$/.test(assignDate)) {
					const timePart =
						assignTime.length === 5 ? `${assignTime}:00` : "11:30:00";
					...
					isoScheduledAt =
						isNaN(parsedDateObj.getTime()) ?
							new Date().toISOString()
						:	parsedDateObj.toISOString();
				}
			} catch {
			...
			const result = await scheduleInterview({
				...
				meetingLinkOrLocation:
					assignMode === "Online" ? assignMeetingLink : undefined,
				roundNumber: selectedAssignStage.id,
			}).unwrap();
					...
					s.id === selectedAssignStage.id ?
						{
							...
							interviewer:
								displayName ||
								(selectedAssignStage.isDirectorRound ? "Director" : (
									"Interviewer"
								)),
							interviewerInitials: initials || "IN",
							interviewerUserId: Number(assignedInterviewer),
			...
			const teamsChatLink = (result as any)?.data?.teamsChatLink;
			toast.success("Interview Scheduled & Invites Sent", {
				description: `${displayName} assigned (${assignMode}). Candidate & Interviewer emails sent from Recruitment@sthapatya.in.`,
				action:
					teamsChatLink ?
						{
							label: "💬 Ping on Teams",
							onClick: () => window.open(teamsChatLink, "_blank"),
						}
					:	undefined,
			});
		} catch (err) {
	...
	const handlePaperAptitudePass = async (stageId: number) => {
		...
		toast.success("Paper Aptitude Passed", {
			description:
				"Candidate passed the Paper Aptitude Test and is advanced to the next round.",
		});
	...
	if (
		isCandidateLoading ||
		(!candidateRes?.data && (isListLoading || isCandidateFetching))
	) {
	...
	return (
		<motion.div
			...
			<div className='grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-5 items-start'>
				...
				<div className='lg:col-span-4 flex flex-col gap-3.5'>
					...
					<motion.div
						...
						className='bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[var(--radius-xl)] p-4 shadow-[var(--shadow-xs)] flex flex-col gap-3.5 relative overflow-hidden'>
						...
						<div className='flex items-start justify-between gap-2.5 border-b border-[var(--border-soft)] pb-3 min-h-[76px]'>
							<div className='flex items-center gap-3 min-w-0'>
								<div className='group relative w-13 h-13 rounded-xl overflow-hidden shrink-0 border border-[var(--border-default)] shadow-2xs bg-[var(--surface-2)] transition-transform hover:scale-[1.02]'>
									{(
										candidate.avatar &&
										candidate.avatar.trim().length > 0 &&
										!candidate.avatar.includes("unsplash")
									) ?
										<img
								...
								<div className='flex flex-col min-w-0'>
									<div className='flex items-center gap-1.5 flex-wrap'>
										<h1 className='font-bold text-[var(--text-primary)] text-base font-heading truncate'>
											{candidate.name}
										</h1>
										<span
						...
						<div className='flex items-center justify-between bg-[var(--surface-2)] border border-[var(--border-default)] rounded-xl px-3.5 py-2.5'>
							...
							<span className='font-bold text-[var(--text-primary)] text-xs truncate max-w-[200px]'>
								{(
									...
									candidate.appliedFor !== "Walk-In Registration"
								) ?
									candidate.appliedFor
								:	"Senior .NET Architect"}
							</span>
						...
						<div className='grid grid-cols-2 gap-x-3.5 gap-y-3 text-xs'>
							<div className='flex flex-col gap-0.5'>
								<span className='text-[var(--text-tertiary)] font-medium text-[11px]'>
									Email Address
								</span>
							...
							<div className='flex flex-col gap-0.5'>
								...
								<span className='font-semibold text-[var(--accent-indigo)]'>
									{candidate.source}
								</span>
							</div>
							...
							<div className='flex flex-col gap-0.5'>
								...
								<span className='font-semibold text-[var(--text-primary)] tabular-figures'>
									...
									{candidate.passingYear ?
										<span className='text-[var(--text-tertiary)] font-normal text-xs tabular-figures'>
											({candidate.passingYear})
										</span>
									:	null}
								</span>
							...
							<div className='col-span-2 bg-[var(--surface-2)] border border-[var(--border-default)] rounded-xl p-3 flex flex-col gap-1.5 mt-0.5'>
								<div className='flex items-center justify-between'>
									<span className='text-[11px] font-bold text-[var(--accent-indigo)] uppercase tracking-wider flex items-center gap-1.5 font-heading'>
										...
										/>
										Reference:{" "}
										{candidate.refType === "Internal" ?
											"Internal Employee Referral"
										: candidate.refType === "External" ?
											"External Referral / Agency"
										: candidate.refName ?
											"Referral"
										:	"Direct Application"}
									</span>
								...
								:	<p className='text-[11px] text-[var(--text-tertiary)] pt-0.5'>
										Candidate applied directly without any internal or external
										referral.
									</p>
								}
							</div>
					...
					<motion.div
						variants={tactilePopCardVariant}
						className='bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[var(--radius-xl)] p-4 shadow-[var(--shadow-xs)] flex flex-col gap-2.5 relative overflow-hidden'>
						{/* Top Inset Highlight Catch */}
						<div className='absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-white/12 to-transparent pointer-events-none' />
								...
								<div className='flex items-center gap-1.5'>
									...
									<motion.label
										...
										<input
											...
											className='hidden'
											onChange={(e) =>
												handleUploadDocumentFile(e, "Application Form")
											}
										/>
									</motion.label>
								</div>
							...
							:	documentsData.map((doc) => (
									<motion.div
										...
										<div className='flex items-center gap-2.5 min-w-0 flex-1'>
											<span
												className={`w-6.5 h-6.5 rounded-md font-bold text-[10px] flex items-center justify-center shrink-0 ${
													doc.type === "Profile Photo" ?
											...
											<div className='flex flex-col min-w-0'>
												<span
													className='font-semibold text-[var(--text-primary)] text-xs truncate'
													title={doc.name}>
													{doc.name}
												</span>
				...
				<div className='lg:col-span-8 flex flex-col gap-4'>
					...
					<motion.div
						variants={tactilePopCardVariant}
						className='bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[var(--radius-xl)] p-4 shadow-[var(--shadow-xs)] relative overflow-hidden'>
						{/* Top Inset Highlight Catch */}
						<div className='absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-white/12 to-transparent pointer-events-none' />
						...
						<div className='flex items-start justify-between gap-2.5 min-h-[76px]'>
							<div className='flex items-center gap-3 min-w-0'>
								...
								<div className='flex flex-col min-w-0'>
									...
									<span className='text-xs text-[var(--text-secondary)] font-medium truncate mt-0.5'>
										Live candidate evaluation timeline &amp; panel feedback
										scorecard history
									</span>
								</div>
							</div>
							...
							<div className='flex items-center gap-2 shrink-0 flex-wrap'>
								{(
									...
									candidate.source?.toLowerCase().includes("walk-in") ||
									(candidate as any)?.registrationChannel === "Walk-in"
								) ?
									<>
										<motion.button
											type='button'
											...
											onClick={() =>
												router.push(
													`/dashboard/candidates/${candidateId}/evaluation?round=aptitude`,
												)
											}
										...
										<motion.button
											...
											onClick={() =>
												router.push(
													`/dashboard/candidates/${candidateId}/evaluation?round=technical`,
												)
											}
					...
					<motion.div
						variants={staggerFastContainer}
						className='relative pl-7 sm:pl-9 flex flex-col gap-4.5'>
							...
							const isAssignedToUser =
								...
								(() => {
									...
									const currentFullName = (
										...
										: typeof window !== "undefined" ?
											localStorage.getItem("step_name") ||
											localStorage.getItem("step_user_name") ||
											""
										:	"")
										.trim()
										.toLowerCase();
									...
									if (
										...
										Number(stage.interviewerUserId) > 0
									) {
										return Number(stage.interviewerUserId) === loggedInId;
									}

									// 2. Fall back to name matching
									const assignedName = (stage.interviewer || "")
										.trim()
										.toLowerCase();
									if (
										!assignedName ||
										assignedName === "unassigned" ||
										assignedName === "talent acquisition" ||
										assignedName === "assigned evaluator"
									) {
										return false;
									}
									...
									return (
										assignedName === currentFullName ||
										assignedName.includes(currentFullName) ||
										currentFullName.includes(assignedName)
									);
								})();
									...
									{!isLast && (
										<div
											className={`absolute -left-7 sm:-left-9 top-8 bottom-[-22px] w-0.5 transform translate-x-[13px] sm:translate-x-[15px] z-0 ${
												isPassed ? "bg-[var(--status-success)]"
												: isInProgress ?
									...
									<div className='absolute -left-7 sm:-left-9 top-4 z-10 flex items-center justify-center'>
										...
										: isInProgress ?
											<span
												...
												title={`Stage ${stage.id}: In-Progress`}>
										...
										:	<span
												className='w-7 h-7 rounded-full bg-[var(--surface-3)] text-[var(--text-tertiary)] border border-[var(--border-default)] font-bold text-xs flex items-center justify-center ring-4 ring-[var(--canvas)]'
												title={`Stage ${stage.id}: Pending`}>
												{stage.id}
											</span>
										}
									</div>
											...
											:	!stage.isTerminated && (
													<div className='flex items-center gap-1.5 sm:gap-2 w-full sm:w-auto flex-wrap sm:ml-auto'>
														...
														: (
															...
															(stage.name.includes("Coding & Algorithm") &&
																!stage.name.includes("F2F"))
														) ?
																	...
																	<div className='flex items-center gap-2 flex-wrap'>
																		<span className='px-2.5 py-1 rounded-lg text-xs font-bold bg-[var(--status-success-bg)] text-[var(--status-success-text)] border border-[var(--status-success-border)] flex items-center gap-1'>
																			...
																			<span>
																				{stage.id === 1 ?
																					"Passed Aptitude (≥ 70%)"
																				:	"Passed Technical Assessment (≥ 70%)"}
																			</span>
																		</span>
																	...
																	<div className='flex items-center gap-2 flex-wrap'>
																		<span className='px-2.5 py-1 rounded-lg text-xs font-bold bg-[var(--status-danger-bg)] text-[var(--status-danger-text)] border border-[var(--status-danger-border)] flex items-center gap-1'>
																			...
																			<span>
																				{stage.id === 1 ?
																					"Failed Aptitude (< 70%)"
																				:	"Failed Technical Assessment (< 70%)"}
																			</span>
																		</span>
																		{isHrOrDirectorOrAdmin &&
																			stage.id === 1 &&
																			((
																				stage.feedback?.includes("Attempt 2") ||
																				stage.feedback?.includes("2 of 2")
																			) ?
														...
														: stage.isOfferRound ?
															...
															<span className='px-2.5 py-1 rounded-lg text-xs font-semibold bg-[var(--surface-2)] text-[var(--text-tertiary)] border border-[var(--border-default)] flex items-center gap-1'>
																...
																/>
																<span>
																	Offer Rollout (Format in Preparation)
																</span>
															</span>
														:	null}
													</div>
									...
									{offerLetterId ?
										<div className='w-full h-9 px-3 rounded-xl border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-primary)] font-semibold flex items-center tabular-figures'>
											{offerRes?.data?.joiningDate ?
												new Date(offerRes.data.joiningDate).toLocaleDateString(
													"en-GB",
										...
										<div className='flex items-center gap-3'>
											...
											<div className='flex flex-col'>
												...
												<span className='text-[10.5px] text-[var(--text-secondary)] font-mono'>
													Prepared by {offerRes?.data?.preparedByName || "…"} •
													Status: {offerRes?.data?.status || "Loading…"}
												</span>
											</div>
										</div>
									...
									{offerRes?.data?.status === "PendingApproval" && (
										<div className='flex flex-col gap-1.5'>
											<label className='text-xs font-bold text-[var(--text-primary)]'>
												Director Security PIN (4 digits)
											</label>
											<input
												type='password'
												inputMode='numeric'
											...
											<span className='text-[11px] text-[var(--text-secondary)]'>
												Only a Director account can approve this — verified
												against the same PIN used for Director login.
											</span>
										</div>
									)}
									{offerRes?.data?.status === "Approved" && (
										<div className='flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--status-success-bg)] border border-[var(--status-success-border)] text-[11px] font-semibold text-[var(--status-success-text)]'>
											...
											<span>
												Approved by{" "}
												{offerRes.data.approvedByName || "the Director"}
												{offerRes.data.approvedAt ?
													` on ${new Date(offerRes.data.approvedAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}`
												:	""}
												.
											</span>
										</div>
				...
				{selectedAssignStage &&
					(() => {
						const isAssessmentRound =
							...
							(selectedAssignStage.name.includes("Coding & Algorithm") &&
								!selectedAssignStage.name.includes("F2F"));
											...
											<>
												...
												{assignMode === "Online" && (
													<div className='animate-in fade-in zoom-in-95'>
														<label className='font-bold text-[var(--text-primary)] block mb-1'>
															Video Meeting Link (Microsoft Teams / Google Meet)
														</label>
														<input
															type='url'
															...
															onChange={(e) =>
																setAssignMeetingLink(e.target.value)
															}
															className='w-full h-9 px-3 rounded-xl border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-primary)] text-xs focus-ring-step'
														/>
													</div>
												)}
											</>
						...
						<motion.form
								...
								<div className='flex items-center gap-3'>
									...
									<div>
										...
										<p className='text-xs text-[var(--text-secondary)] font-medium'>
											...
											:	"Interviewer"}
											:{" "}
											{selectedFeedbackStage.interviewer ||
												"Assigned Evaluator"}
										</p>
									</div>
								</div>
							...
							{selectedFeedbackStage.isDirectorRound ?
								<>
									...
									<div className='flex flex-col gap-1.5'>
										<div className='flex items-center justify-between'>
											<label className='text-xs font-bold text-[var(--text-primary)]'>
												Remarks &amp; Detailed Rationale
											</label>
											<span
												className={`text-[11px] font-mono font-semibold ${feedbackText.length >= 480 ? "text-[var(--status-danger)]" : "text-[var(--text-tertiary)]"}`}>
									...
									{directorDecision !== "hold" && (
										<div className='flex flex-col gap-1.5 pt-2 border-t border-[var(--border-soft)]'>
											...
											/>
											<span className='text-[11px] text-[var(--text-secondary)]'>
												Enter the Director security PIN to authorize this hiring
												decision.
											</span>
										</div>
									)}
							...
							:	<>
									{myExistingScorecard && (
										<div className='flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--accent-indigo-dim)] border border-[var(--accent-indigo)]/30 text-[11px] font-semibold text-[var(--accent-indigo)]'>
											...
											/>
											<span>
												You already submitted a scorecard on{" "}
												{new Date(
													myExistingScorecard.submittedAt,
										...
										{(
											[
												[
													"Technical",
													scorecardTechnical,
													setScorecardTechnical,
												],
												[
													"Problem Solving",
													scorecardProblemSolving,
													setScorecardProblemSolving,
												],
											] as const
												...
												<div className='grid grid-cols-5 gap-1'>
													{[1, 2, 3, 4, 5].map((n) => (
														<button
															...
															type='button'
															onClick={() => setValue(n)}
															className={`h-8 rounded-lg text-xs font-bold border transition-all cursor-pointer ${
																value === n ?
							...
							<div className='flex items-center justify-end gap-2 pt-3 border-t border-[var(--border-soft)]'>
								...
								<button
									type='submit'
									disabled={
										selectedFeedbackStage.roundType === "Assessment" ? false
										: selectedFeedbackStage.isDirectorRound ?
											(directorDecision !== "hold" &&
												directorPin.length !== 4) ||
											isPublishingDecision
							...
							<div className='flex items-center justify-between border-b border-[var(--border-soft)] pb-3'>
								<div className='flex items-center gap-2.5'>
									...
									<div className='flex flex-col'>
										...
										<span className='text-xs text-[var(--text-secondary)] font-medium'>
											Update personal, professional, academic, and reference
											details below
										</span>
									</div>
								</div>
							...
							<div className='grid grid-cols-1 sm:grid-cols-2 gap-3.5 text-xs'>
								...
								<div>
									...
									<FormSelect
										...
										onChange={(val) =>
											setEditProfileForm((p) => ({ ...p, gender: val }))
										}
								...
								<div>
									...
									<FormSelect
										...
										onChange={(val) =>
											setEditProfileForm((p) => ({ ...p, source: val }))
										}
								...
								<div>
									...
									<FormSelect
										...
										onChange={(val) =>
											setEditProfileForm((p) => ({ ...p, noticePeriod: val }))
										}
								...
								<div>
									...
									<FormSelect
										value={
											editProfileForm.education || "B.Tech in Computer Science"
										}
										onChange={(val) =>
											setEditProfileForm((p) => ({ ...p, education: val }))
										}
								...
								:	<div className='sm:col-span-2 p-2.5 rounded-xl bg-[var(--surface-2)] border border-[var(--border-default)] text-[var(--text-tertiary)] text-xs'>
										Direct candidate application — no referral verification
										details required.
									</div>
								}
							</div>
			...
			{showScheduleTestModal && (
				<CandidateExamPassModal
					...
					candidate={{
						...
						code:
							candidate.code ||
							(candidate as any).candidateCode ||
							`CND-2026-${candidate.id}`,
						name: candidate.name,
						...
						<CandidateAssessmentEvaluationView
							...
							onFinalizeScore={() => {
								...
								toast.success("Assessment Published", {
									description:
										"Assessment results and final score updated successfully.",
								});
							}}
						/>
					</div>
						...
						<motion.div
							...
							<div className='flex items-center justify-between border-b border-[var(--border-soft)] pb-3'>
								<div className='flex items-center gap-2'>
									...
									<div>
										<h3 className='font-bold text-[var(--text-primary)] text-sm font-heading'>
											{selectedDocPreview.name}
										</h3>
										<p className='text-[11px] text-[var(--text-secondary)] font-mono tabular-figures'>
											{selectedDocPreview.type} • {selectedDocPreview.size} •
											Attached {selectedDocPreview.date}
										</p>
									</div>
								</div>
								<button
							...
							<div className='bg-[var(--surface-2)] border border-[var(--border-default)] rounded-[var(--radius-lg)] p-6 flex flex-col items-center justify-center text-center min-h-[220px]'>
								...
								:	<div className='flex flex-col items-center gap-2.5'>
										...
										<span className='text-[var(--text-secondary)] text-[11.5px] max-w-sm'>
											Verified candidate official document persisted in SQL
											Server database.
										</span>
									</div>
								}
							...
							<div className='flex items-center justify-between pt-2'>
								<button
									type='button'
									onClick={() =>
										handleDownloadDocument(selectedDocPreview.name)
									}
			...
			<DirectorAccessShareModal
				...
				candidateName={candidate.name}
				candidateCode={
					(candidate as any).candidateCode || `CND-2026-${numericId}`
				}
