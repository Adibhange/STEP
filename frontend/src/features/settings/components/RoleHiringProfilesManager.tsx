...
import type { AssessmentBlueprint } from "@/store/services/types";
...
export const RoleHiringProfilesManager: React.FC = () => {
	...
	const { data: blueprintsRes, isLoading: isBlueprintsLoading } =
		useGetBlueprintsV2Query();
	...
	const [editingTemplate, setEditingTemplate] =
		useState<Partial<AssessmentBlueprint> | null>(null);
	...
	const handleSaveTemplate = async (e: React.FormEvent) => {
		...
		try {
			const payload = {
				id:
					editingTemplate.id && editingTemplate.id < 1000000000 ?
						editingTemplate.id
					:	undefined,
				code: editingTemplate.code,
				name: editingTemplate.name.trim(),
				defaultPassingPercentage:
					Number(editingTemplate.defaultPassingPercentage) || 70,
				...
				sectionRules: (editingTemplate.sectionRules || []).map((r, idx) => ({
					...
					timeLimitMinutes:
						r.timeLimitMinutes ? Number(r.timeLimitMinutes) : null,
					selectionStrategy: r.selectionStrategy || "RandomShuffled",
					displayOrder: idx + 1,
				})),
			};
			...
			toast.success(
				editingTemplate.id ? "Template Updated" : "Template Created",
				{
			...
			toast.error("Save Failed", {
				description:
					err?.data?.message ||
					"Could not save assessment template to database.",
			});
		}
	};
	...
	const handleDeleteTemplate = async (id: number) => {
		try {
			...
			toast.success("Template Removed", {
				description: "Assessment template deleted from database.",
			});
		} catch (err: any) {
			toast.error("Delete Failed", {
				description:
					err?.data?.message || "Could not delete assessment template.",
			});
		}
	};
	...
	return (
		<motion.div
				...
				<div className='flex flex-wrap items-center justify-between gap-3 p-3.5 border-b border-[var(--border-default)] bg-[var(--surface-1)]'>
					<div className='flex flex-col gap-0.5'>
						...
						<p className='text-[11px] text-[var(--text-tertiary)] ml-6'>
							Define reusable test templates. Templates are selected when
							creating a vacancy.
						</p>
					</div>
									...
									{tmpl.sectionRules.map((rule, rIdx) => (
										<div
											...
											<div className='flex items-center gap-2'>
												<Icon
													name={
														rule.sectionType === "SQLQuery" ? "file-text"
											...
											<div className='flex items-center gap-1.5 font-mono text-[11px] text-[var(--text-tertiary)]'>
												...
												{rule.timeLimitMinutes && (
													<span>({rule.timeLimitMinutes}m)</span>
												)}
											</div>
										</div>
									))}
								...
								<div className='flex items-center gap-3 text-xs font-mono'>
									<span className='text-[var(--text-secondary)]'>
										...
										<strong className='text-[var(--text-primary)]'>
											{tmpl.totalQuestions} Qs
										</strong>
									</span>
									<span className='text-[var(--text-secondary)]'>
										Pass:{" "}
										<strong className='text-[var(--status-success-text)]'>
											{tmpl.defaultPassingPercentage}%
										</strong>
									</span>
									<span className='text-[var(--text-secondary)]'>
										<strong className='text-[var(--text-primary)]'>
			...
			<EnterpriseModal
				...
				subtitle='Configure round structure, blueprints, and evaluation thresholds.'
				icon='file-text'
				maxWidth='2xl'
						...
						<div className='grid grid-cols-1 sm:grid-cols-3 gap-3'>
							...
							<div className='sm:col-span-2 space-y-1'>
								...
								<input
									...
									className='w-full h-9.5 px-3 rounded-xl border border-[var(--border-default)] bg-[var(--surface-2)] text-xs font-semibold text-[var(--text-primary)] outline-none focus:border-[var(--accent-indigo)]'
								/>
							</div>
							...
							<div className='space-y-1'>
								...
								<input
									...
									onChange={(e) =>
										setEditingTemplate({
											...editingTemplate,
											defaultPassingPercentage: Number(e.target.value),
										})
									}
									className='w-full h-9.5 px-3 rounded-xl border border-[var(--border-default)] bg-[var(--surface-2)] text-xs font-mono font-bold text-[var(--status-success-text)] outline-none focus:border-[var(--accent-indigo)]'
								/>
							</div>
						</div>
						...
						<div className='space-y-2 border-t border-[var(--border-default)] pt-3'>
							<div className='flex items-center justify-between'>
								<div>
									<label className='text-[11px] font-bold text-[var(--text-tertiary)] uppercase font-mono block'>
										Assessment Sections (
										{editingTemplate.sectionRules?.length || 0})
									</label>
									<span className='text-[10px] text-[var(--text-tertiary)]'>
										Questions are filtered by candidate's experience tier at
										runtime
									</span>
								</div>
								<button
									...
									className='px-2.5 py-1 rounded-lg bg-[var(--surface-2)] hover:bg-[var(--surface-hover)] text-xs font-bold text-[var(--accent-indigo)] border border-[var(--border-default)] cursor-pointer'>
									+ Add Section
								</button>
							</div>
							...
							<div className='space-y-2'>
								{(editingTemplate.sectionRules || []).map((rule, idx) => (
									<div
										key={idx}
										className='p-3 rounded-xl bg-[var(--surface-2)] border border-[var(--border-default)] space-y-2'>
										...
										<div className='grid grid-cols-2 sm:grid-cols-4 gap-2'>
											...
											<div>
												...
												<input
													...
													className='w-full h-8.5 px-2 rounded-lg border border-[var(--border-default)] bg-[var(--surface-1)] text-[11px] font-mono font-bold outline-none'
												/>
											</div>
