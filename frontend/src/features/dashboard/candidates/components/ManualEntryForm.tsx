...
interface ManualEntryFormProps {
	onSuccess: (
		candidate: Partial<DashboardCandidate>,
		addAnother?: boolean,
	) => void;
	onCancel: () => void;
...
export const ManualEntryForm: React.FC<ManualEntryFormProps> = ({
	...
	const [candidateType, setCandidateType] = useState<"experienced" | "fresher">(
		"experienced",
	);
	...
	const [registerCandidateApi, { isLoading: isSubmitting }] =
		useRegisterCandidateMutation();
	const [uploadCandidateDocumentApi] = useUploadCandidateDocumentMutation();

	// Open vacancies
	const { data: vacanciesRes } = useGetVacanciesQuery({
		pageSize: 200,
		status: "Open",
	});
	...
	const selectedVacancyEntity = useMemo(() => {
		return (
			(vacanciesRes?.data || []).find((v: any) => Number(v.id) === vacancyId) ||
			null
		);
	...
	const isDuplicateEmail = useMemo(
		() =>
			...
			(emailCheckRes?.data || []).some(
				(c: any) =>
					(c.email || "").toLowerCase() === debouncedEmail.toLowerCase(),
			),
	...
	const isDuplicatePhone = useMemo(
		() =>
			...
			(phoneCheckRes?.data || []).some(
				(c: any) => (c.phone || "") === debouncedPhone,
			),
	...
	const validateForm = () => {
		if (!firstName.trim()) {
			toast.error("First Name Required", {
				description: "Please enter candidate first name.",
			});
			return false;
		}
		...
		if (!lastName.trim()) {
			toast.error("Last Name Required", {
				description: "Please enter candidate last name.",
			});
			return false;
		}
		if (lastName.trim().length > 50) {
			toast.error("Last Name Too Long", {
		...
		if (!vacancyId) {
			toast.error("Vacancy Required", {
				description:
					"Please select which open vacancy this candidate is applying for.",
			});
			return false;
		}
		const isReferralChannel = hasReferral || source === "Referral";
		if (isReferralChannel && !refName.trim()) {
			toast.error("Referral Details Required", {
				description:
					"Please enter referrer employee name for referral candidate.",
			});
			return false;
		}
		return true;
	...
	const handleSave = async (addAnother = false) => {
		...
		try {
			...
			const isReferralChannel = hasReferral || source === "Referral";
			const channel =
				isReferralChannel ? "Referral"
				: (
					["Walk-in", "Office", "Referral", "Portal", "Recruiter"].includes(
						source,
					)
				) ?
					source
			...
			const expCTCNum =
				expectedCtc.trim() ? parseFloat(expectedCtc) : undefined;
			...
			const newCand: Partial<DashboardCandidate> = {
				...
				experience:
					candidateType === "fresher" ? "Fresher" : `${totalExp} Years`,
			...
			<div className='flex-1 min-h-0 px-4 sm:px-8 pt-4 pb-6 space-y-4 overflow-y-auto scrollbar-none'>
							...
							<div className='flex items-center justify-between gap-2'>
								<div>
									<p className='text-xs font-bold text-[var(--text-primary)] font-heading'>
										...
										<span className='text-[10.5px] font-normal text-[var(--text-tertiary)]'>
											(Optional)
										</span>
									</p>
						...
						<div>
							...
							<div className='w-full h-10 px-3.5 rounded-xl border border-[var(--border-default)] bg-[var(--surface-1)] flex items-center justify-between text-xs sm:text-[13px] font-medium text-[var(--text-primary)] shadow-2xs'>
								<span className='truncate'>
									{selectedVacancyEntity?.title || "Select a vacancy above..."}
								</span>
							...
							<div className='flex items-center gap-3 min-w-0'>
								...
								<div className='min-w-0'>
									...
									<p className='text-[11px] text-emerald-500 font-medium mt-0.5'>
										{(resumeFile.size / 1024).toFixed(1)} KB • Document Attached
										Ready
									</p>
								</div>
							</div>
				...
				<motion.div
					...
					<button
						...
						<Icon
							name={hasReferral ? "chevron-up" : "chevron-down"}
							size='xs'
									...
									<div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
										...
										<div>
											...
											<input
												...
												value={refMobile}
												onChange={(e) =>
													setRefMobile(e.target.value.replace(/\D/g, ""))
												}
												className='w-full h-10 px-3.5 rounded-xl border border-[var(--border-default)] focus:border-[var(--accent-indigo)] text-xs sm:text-[13px] bg-[var(--surface-1)] text-[var(--text-primary)] outline-none transition-all'
											/>
										</div>
