...
export const StatsCard: React.FC<StatsCardProps> = ({
	...
	return (
		<div
			onClick={onClick}
			style={{
				background: `linear-gradient(180deg, var(--surface-1) 30%, ${bgColor} 100%)`,
				boxShadow: "var(--inset-highlight)",
			}}
			className={`
        group relative flex flex-col justify-between p-4 rounded-lg border
        transition-all duration-200 ease-out cursor-pointer select-none overflow-hidden
        hover:-translate-y-1 active:scale-[0.98]
        ${
					active ?
						"border-accent-indigo shadow-md ring-1 ring-accent-indigo/30"
					:	"border-border-default hover:border-accent-indigo/40 hover:shadow-[0_12px_24px_-6px_rgba(0,0,0,0.35),0_0_16px_-4px_rgba(99,102,241,0.18)]"
				}
      `}>
			{/* 3px Top Accent Indicator Line with subtle shine */}
			<div
				className='absolute top-0 left-0 right-0 h-0.75 transition-all duration-200 group-hover:h-[3.5px]'
			...
			<div className='flex items-center justify-between gap-1.5 mb-1.5 pt-0.5'>
				<div className='flex items-center gap-2 min-w-0'>
					<span
						className='size-7 rounded-lg flex items-center justify-center shrink-0 transition-transform duration-200 ease-out group-hover:scale-110 shadow-xs'
					...
					<h3 className='text-[13px] font-bold text-text-primary font-heading leading-tight truncate'>
						{title}
					</h3>
				</div>
			...
			<div className='flex flex-col mt-1'>
				<span className='text-[24px] font-black text-text-primary font-mono tracking-tight leading-none group-hover:text-accent-indigo transition-colors duration-200'>
					<AnimatedCounter value={count} />
				</span>
				<span className='text-[11.5px] font-medium text-text-secondary mt-1.5 truncate'>
					{subtitle}
				</span>
			</div>
		</div>
