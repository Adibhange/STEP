...
export const CandidateAssessmentEvaluationView: React.FC<
	CandidateAssessmentEvaluationViewProps
> = ({
	candidateId = "",
	...
	const [evaluateAnswerApi, { isLoading: isSavingAnswer }] =
		useEvaluateAnswerMutation();
	const [publishResultApi, { isLoading: isPublishing }] =
		usePublishAssessmentResultMutation();
	...
	const selectedAnswer = useMemo(
		() =>
			answers.find((a) => a.candidateExamAnswerId === selectedAnswerId) ??
			answers[0],
	...
	const roundGroups = useMemo(() => {
		...
		answers.forEach((a) => {
			const sectionKey =
				...
				(isMcqType(a.questionType) ? "Multiple Choice (MCQ)"
				: a.questionType === "CODING" ? "Coding Challenges"
				: a.questionType === "SQL" ? "SQL Queries & Schema"
	...
	const autoMcqScore = useMemo(() => {
		const sum = answers
			.filter((a) => isMcqType(a.questionType))
			.reduce((acc, a) => acc + a.marksObtained, 0);
		return Math.round(sum * 100) / 100;
	}, [answers]);
	...
	const allNonMcqEvaluated =
		nonMcqAnswers.length === 0 ||
		nonMcqAnswers.every(
			(a) => a.evaluationStatus === "Evaluated" || a.evaluationLocked,
		);
	const isPublished =
	...
	const handleSaveAnswer = async () => {
		...
		const marks = Math.min(
			selectedAnswer.marks,
			Math.max(0, marksDraft[selectedAnswer.candidateExamAnswerId] ?? 0),
		);
		try {
			await evaluateAnswerApi({
				...
				marksObtained: marks,
				evaluatorRemarks:
					notesDraft[selectedAnswer.candidateExamAnswerId] || undefined,
			}).unwrap();
			...
			toast.error("Save Failed", {
				description:
					err?.data?.message || "Could not save marks for this question.",
			});
		}
	};
	...
	const handlePublish = async () => {
		...
		try {
			const res = await publishResultApi({
				sessionId: querySessionId,
			}).unwrap();
			...
			onFinalizeScore?.(
				Number(res.data.totalScore),
				Number(res.data.percentage),
				passed ? "Passed" : "Failed",
			);
		} catch (err: any) {
			toast.error("Publish Failed", {
				description: err?.data?.message || "Could not publish this result.",
			});
		}
	};
		...
		return (
			<div className='min-h-screen bg-[var(--canvas,#f7f8fb)] flex items-center justify-center p-6 text-center'>
				<div className='bg-white border border-amber-200 rounded-2xl p-8 shadow-xl max-w-md w-full flex flex-col items-center gap-3'>
					...
					/>
					<h2 className='text-lg font-extrabold text-amber-700 font-heading'>
						No Assessment Session Yet
					</h2>
					<p className='text-xs text-slate-500'>
						This candidate hasn&apos;t started (or been assigned) an assessment
						round, so there&apos;s nothing to evaluate yet.
					</p>
					<button
						type='button'
		...
		return (
			<div className='min-h-screen bg-[var(--canvas,#f7f8fb)] flex items-center justify-center p-6 text-center'>
				<div className='bg-white border border-rose-200 rounded-2xl p-8 shadow-xl max-w-md w-full flex flex-col items-center gap-3'>
					...
					/>
					<h2 className='text-lg font-extrabold text-rose-700 font-heading'>
						Could Not Load Evaluation
					</h2>
					<p className='text-xs text-slate-500'>
						This assessment session could not be loaded.
					</p>
				</div>
			</div>
		);
						...
						{assessmentRounds.map((rnd) => {
							...
							return (
								<button
									...
									type='button'
									onClick={() => {
										if (rnd.candidateExamSessionId)
											setActiveSessionId(rnd.candidateExamSessionId);
									}}
									...
									<Icon
										name={rnd.roundNumber === 1 ? "file-text" : "layers"}
										size='xs'
									/>
				...
				<aside className='w-80 sm:w-96 bg-[var(--surface-1)] border-r border-[var(--border-default)] p-4 flex flex-col gap-4 overflow-y-auto shrink-0 scrollbar-thin'>
					...
					<div className='p-3.5 rounded-xl bg-[var(--surface-2)] border border-[var(--border-default)] flex flex-col gap-2.5 shadow-2xs'>
						<div className='flex items-center justify-between text-xs'>
							...
							<span className='font-mono font-bold text-[var(--text-primary)]'>
								{timeUsedMinutes !== null ?
									`${timeUsedMinutes} / ${evaluation.frozenTotalDurationMinutes} Mins`
								:	`— / ${evaluation.frozenTotalDurationMinutes} Mins`}
							</span>
						</div>
						...
						<div className='grid grid-cols-2 gap-2 text-[10.5px] font-mono border-t border-[var(--border-soft)] pt-2'>
							<div className='flex flex-col'>
								...
								<span
									className={`font-bold ${evaluation.tabSwitchWarnings > 0 ? "text-[var(--status-warning)]" : "text-[var(--status-success)]"}`}>
							...
							<div className='flex flex-col'>
								...
								<span
									className={`font-bold ${evaluation.assessmentIntegrityScore < 100 ? "text-[var(--status-warning)]" : "text-[var(--status-success)]"}`}>
									{evaluation.assessmentIntegrityScore}/100
								</span>
							</div>
						</div>

						{/* Proctoring Timeline Log */}
						{evaluation.proctoringLogs &&
							evaluation.proctoringLogs.length > 0 && (
								<div className='mt-2 pt-2 border-t border-[var(--border-soft)] max-h-32 overflow-y-auto scrollbar-thin'>
									<span className='text-[10px] text-[var(--text-tertiary)] uppercase font-bold mb-1 block'>
										Timeline
									</span>
									<div className='flex flex-col gap-1.5'>
										{evaluation.proctoringLogs.map((log) => (
											<div
												key={log.id}
												className='flex items-start justify-between text-[10px] font-mono p-1 rounded bg-[var(--surface-3)]'>
												<div className='flex flex-col'>
												...
												{log.penaltyApplied > 0 && (
													<span className='text-[var(--status-danger-text)] font-bold'>
														-{log.penaltyApplied}
													</span>
												)}
											</div>
							...
							<div
								...
								<div className='flex items-center justify-between text-[11px] font-bold text-[var(--text-secondary)] border-b border-[var(--border-default)] pb-1'>
									...
									<span className='text-[10px] font-mono text-[var(--accent-indigo)]'>
										{isMcqType(group.answers[0].questionType) ?
											"Auto MCQ"
										:	"Manual Grade"}
									</span>
								</div>
								...
								<div className='grid grid-cols-5 gap-2'>
									{group.answers.map((a) => {
										const isSelected =
											a.candidateExamAnswerId ===
											selectedAnswer.candidateExamAnswerId;
										...
										return (
											<button
												...
												type='button'
												onClick={() =>
													setSelectedAnswerId(a.candidateExamAnswerId)
												}
					...
					<div className='mt-auto p-4 rounded-xl bg-[var(--surface-2)] border border-[var(--border-default)] flex flex-col gap-3 shadow-2xs'>
						...
						:	<button
								...
								title={
									!allNonMcqEvaluated ?
										"All coding/SQL/subjective answers must be evaluated first"
									:	undefined
								}
								...
								<span>
									{isPublishing ? "Publishing…" : "Publish Result & Lock"}
								</span>
							</button>
						}
				...
				<main className='flex-1 p-6 overflow-y-auto flex flex-col gap-5 scrollbar-thin bg-[var(--canvas)]'>
					...
					<div className='flex items-start justify-between border-b border-[var(--border-default)] pb-3 flex-wrap gap-3'>
						...
						<div className='flex flex-col items-end shrink-0 font-mono'>
							...
							<span
								className={`text-[10px] font-bold px-2 py-0.2 rounded mt-0.5 ${isMcqType(selectedAnswer.questionType) ? "bg-[var(--status-success-bg)] text-[var(--status-success-text)] border border-[var(--status-success-border)]" : "bg-[var(--status-warning-bg)] text-[var(--status-warning-text)] border border-[var(--status-warning-border)]"}`}>
						...
						<div className='flex flex-col gap-3'>
							<div className='p-3 rounded-xl bg-[var(--surface-2)] border border-[var(--border-default)] text-xs font-semibold flex items-center justify-between gap-2 flex-wrap'>
								<div className='flex items-center gap-2 text-[var(--status-success-text)]'>
									...
									/>
									<span>
										Auto-Evaluated by System Engine. Correct options are scored
										automatically.
									</span>
								</div>
							...
							{selectedAnswer.selectedOptionIds.length === 0 && (
								<div className='p-3 rounded-xl bg-[var(--status-warning-bg)]/40 border border-[var(--status-warning-border)] text-[var(--status-warning-text)] text-xs font-medium flex items-center gap-2'>
									<Icon
										name='alert-triangle'
										size='xs'
										className='shrink-0 text-[var(--status-warning-text)]'
									/>
									<span>
										Candidate did not select any option for this question.
									</span>
								</div>
							)}
							...
							<div className='grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-1'>
								{selectedAnswer.options.map((opt) => {
									const isSelected = selectedAnswer.selectedOptionIds.includes(
										opt.id,
									);
					...
					{!isMcqType(selectedAnswer.questionType) && (
						<div className='flex flex-col gap-3'>
							<div className='flex items-center justify-between text-xs font-mono text-[var(--text-secondary)]'>
								<span>
									Candidate Submitted {selectedAnswer.questionType} Solution:
								</span>
							</div>
							...
							<pre className='p-4 rounded-xl bg-[#0d1117] text-emerald-400 border border-[var(--border-default)] font-mono text-xs leading-relaxed overflow-x-auto scrollbar-thin max-h-80 shadow-2xs'>
								<code>
									{selectedAnswer.submittedAnswerText ||
										"(No answer submitted)"}
								</code>
							</pre>
						</div>
						...
						<div className='mt-auto p-5 rounded-xl bg-[var(--surface-1)] border border-[var(--border-default)] flex flex-col gap-4 shadow-2xs'>
							...
							<div className='grid grid-cols-1 sm:grid-cols-3 gap-4 items-start'>
								<div className='flex flex-col gap-1.5'>
									...
									<div className='flex items-center gap-2'>
										<input
											...
											value={
												marksDraft[selectedAnswer.candidateExamAnswerId] ??
												selectedAnswer.marksObtained
											}
											onChange={(e) =>
												setMarksDraft((prev) => ({
													...prev,
													[selectedAnswer.candidateExamAnswerId]: Math.min(
														selectedAnswer.marks,
								...
								<div className='sm:col-span-2 flex flex-col gap-1.5'>
									...
									<textarea
										value={
											notesDraft[selectedAnswer.candidateExamAnswerId] ??
											selectedAnswer.evaluatorRemarks ??
											""
										}
										disabled={selectedAnswer.evaluationLocked}
										onChange={(e) =>
											setNotesDraft((prev) => ({
												...prev,
												[selectedAnswer.candidateExamAnswerId]: e.target.value,
											}))
										}
