...
export const AssessmentsView: React.FC = () => {
  ...
  const submissions: CandidateAssessmentRow[] = useMemo(() => {
    return (candidatesRes?.data || []).map((c: any, idx: number) => ({
      ...
      candidateName: `${c.firstName || ''} ${c.lastName || ''}`.trim() || 'Candidate',
      ...
      submittedAt: c.createdAt ? new Date(c.createdAt).toISOString().split('T')[0] : '',
    }));
  }, [candidatesRes]);
        ...
        <div className="border border-[var(--border-default)] rounded-xl overflow-hidden shadow-2xs">
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-[var(--border-default)] font-medium text-[var(--text-primary)]">
              ...
              {isError && (
                <tr>
                  <td colSpan={7} className="py-4 text-center text-xs text-[var(--status-danger-text)] bg-[var(--status-danger-bg)] font-semibold">
              ...
              {!isLoading && !isError && submissions.length === 0 && (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-xs text-[var(--text-tertiary)] font-mono">
