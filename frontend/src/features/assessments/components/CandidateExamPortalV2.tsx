...
const SECTION_TYPE_MAP: Record<
	string,
	{
		title: string;
		shortTitle: string;
		icon: string;
		sectionType: RoundSectionConfig["sectionType"];
	}
> = {
	SINGLE_CHOICE: {
...
export const CandidateExamPortalV2: React.FC<CandidateExamPortalV2Props> = ({
	...
	// ── Exam Flow Lifecycle ─────────────────────────────────────────────────────
	const [examStep, setExamStep] = useState<
		"login" | "instructions" | "active" | "submitted"
	...
	const [eliminatedOptionsMap, setEliminatedOptionsMap] = useState<
		Record<number, number[]>
	>({});
	const [paletteFilter, setPaletteFilter] = useState<
		"ALL" | "ANSWERED" | "UNANSWERED" | "FLAGGED"
	>("ALL");
	...
	const [justSavedQuestionId, setJustSavedQuestionId] = useState<number | null>(
		null,
	);
	const [showSchemaModal, setShowSchemaModal] = useState<boolean>(false);

	// ── Timers & HUD ────────────────────────────────────────────────────────────
	const [totalTimeLeftSeconds, setTotalTimeLeftSeconds] =
		useState<number>(3600);
	const [initialTotalDuration, setInitialTotalDuration] =
		useState<number>(3600);
	const [isTimerRunning, setIsTimerRunning] = useState<boolean>(false);
	const [sectionTimeLeftMap, setSectionTimeLeftMap] = useState<
		Record<number, number>
	>({});
	...
	// ── Security & Proctoring ───────────────────────────────────────────────────
	const [lockedSectionIds, setLockedSectionIds] = useState<Set<number>>(
		new Set(),
	);
	const [tabSwitchWarnings, setTabSwitchWarnings] = useState<number>(0);
	const [showWarningModal, setShowWarningModal] = useState<boolean>(false);
	const [isFullscreen, setIsFullscreen] = useState<boolean>(false);
	const [fullscreenReentrySeconds, setFullscreenReentrySeconds] = useState<
		number | null
	>(null);
	...
	const [syncStatusText, setSyncStatusText] =
		useState<string>("Live Sync Active");

	// ── RTK Mutations ───────────────────────────────────────────────────────────
	const [startSessionApi, { isLoading: isStartingSession }] =
		useStartExamSessionMutation();
	const [saveAnswerBatchApi] = useSaveExamAnswerBatchV2Mutation();
	const [submitExamApi, { isLoading: isSubmittingExam }] =
		useSubmitExamMutation();
	...
	const sections: RoundSectionConfig[] = useMemo(() => {
		...
		const sectionBuckets: Record<
			string,
			{
				config: (typeof SECTION_TYPE_MAP)[string];
				questions: { q: ExamQuestionData; idx: number }[];
			}
		> = {};
		...
		const totalExamMarks =
			rawQuestions.reduce((acc, q) => acc + q.marks, 0) || 1;
		...
		return Object.values(sectionBuckets).map(({ config, questions }) => {
			...
			const durationMinutes =
				isSingleSection ? totalExamMinutes : (
					Math.min(
						totalExamMinutes,
						Math.max(
							5,
							Math.round(totalExamMinutes * (sectionMarks / totalExamMarks)),
						),
					)
				);
	...
	const currentQuestion = useMemo(
		() => rawQuestions[activeQuestionIndex] || null,
		[rawQuestions, activeQuestionIndex],
	);
	...
	const activeSection = useMemo(() => {
		...
		return (
			sections.find((sec) =>
				sec.questionIndices.includes(activeQuestionIndex),
			) ||
			sections[0] ||
			null
		);
	...
	const completionPercentage =
		rawQuestions.length > 0 ?
			Math.round((answeredCount / rawQuestions.length) * 100)
		:	0;
	const sectionCompletionPercentage =
		activeSection && activeSection.questionCount > 0 ?
			Math.round(
				(activeSectionAnsweredCount / activeSection.questionCount) * 100,
			)
		:	0;
		...
		(violationType: string) => {
			setTabSwitchWarnings((prev) => {
				...
				if (next >= 3) {
					...
					setTimeout(() => {
						handleFinalSubmit(
							"Security violation threshold reached (3 tab switch violations). Test automatically locked.",
						);
					}, 150);
				}
				return next;
	...
	// ── Enter Fullscreen on Any User Gesture ──────────────────────────────────
	...
	// ── Auto Fullscreen Keydown & Click Gesture Capture ───────────────────────
	useEffect(() => {
		if (
			fullscreenReentrySeconds === null ||
			isFullscreen ||
			examStep !== "active"
		)
			return;
		...
		window.addEventListener("pointerdown", handleKeyOrClick);

		return () => {
			window.removeEventListener("keydown", handleKeyOrClick);
			window.removeEventListener("pointerdown", handleKeyOrClick);
		};
	}, [fullscreenReentrySeconds, isFullscreen, examStep, requestAppFullscreen]);
	...
	useEffect(() => {
		...
		const timer = setTimeout(() => {
			setFullscreenReentrySeconds((prev) =>
		...
		const interval = setInterval(() => {
			...
			setTotalTimeLeftSeconds((prevTotal) => {
				...
				if (nextTotal <= 0) {
					...
					handleFinalSubmitRef.current(
						"Total examination time expired. Assessment automatically submitted.",
					);
					return 0;
				}
			...
			if (activeSection && !lockedSectionIds.has(activeSection.sectionId)) {
				setSectionTimeLeftMap((prevMap) => {
					const currentSecLeft =
						prevMap[activeSection.sectionId] ??
						activeSection.durationMinutes * 60;
					const nextSecLeft = currentSecLeft - 1;

					if (nextSecLeft <= 0) {
						// Lock current section
						setLockedSectionIds((prevLocked) => {
							const updatedLocked = new Set(prevLocked).add(
						...
						const currentSecIdx = sections.findIndex(
							(s) => s.sectionId === activeSection.sectionId,
						);
						const nextSec =
							currentSecIdx >= 0 && currentSecIdx < sections.length - 1 ?
								sections[currentSecIdx + 1]
							:	null;
						...
						if (nextSec) {
							...
							handleFinalSubmitRef.current(
								`Time expired for final Section ${activeSection.sectionId} (${activeSection.shortTitle}). Assessment submitted.`,
							);
						}
			...
			if (res.success && res.data) {
				...
				try {
					const cached = localStorage.getItem(
						`step_studio_v2_${res.data.sessionToken}`,
					);
					if (cached) {
						const parsed = JSON.parse(cached);
						if (parsed.answersMap)
							setAnswersMap((prev) => ({ ...prev, ...parsed.answersMap }));
						if (parsed.activeQuestionIndex !== undefined)
							setActiveQuestionIndex(parsed.activeQuestionIndex);
						if (parsed.sectionTimeLeftMap)
							setSectionTimeLeftMap(parsed.sectionTimeLeftMap);
						if (parsed.lockedSectionIds)
							setLockedSectionIds(new Set(parsed.lockedSectionIds));
						if (parsed.flaggedQuestionIds)
							setFlaggedQuestionIds(new Set(parsed.flaggedQuestionIds));
						if (parsed.eliminatedOptionsMap)
							setEliminatedOptionsMap(parsed.eliminatedOptionsMap);
					}
				} catch (e) {
				...
				setLoginError(
					res.message ||
						"Invalid examination credentials or expired test link.",
				);
			}
		} catch (err: any) {
			const errMsg =
				...
				:	null) ||
				(err?.data?.message && err.data.message !== "Validation failed" ?
					err.data.message
				:	null) ||
				err?.data?.title ||
				err?.error ||
	...
	const handleToggleEliminateOption = (
		questionId: number,
		optionId: number,
		e: React.MouseEvent,
	) => {
	...
	useEffect(() => {
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (["1", "2", "3", "4", "5"].includes(key))
				chosenIndex = parseInt(key) - 1;
			else if (["A", "B", "C", "D", "E"].includes(key))
				chosenIndex = key.charCodeAt(0) - 65;
	...
	const nextSection = useMemo(() => {
		...
		return currentSecIdx < sections.length - 1 ?
				sections[currentSecIdx + 1]
			:	null;
	...
	const isLastQuestionOfSection =
		activeSection &&
		activeSection.questionIndices[activeSection.questionIndices.length - 1] ===
			activeQuestionIndex;
	const isFirstQuestionOfSection =
		activeSection && activeSection.questionIndices[0] === activeQuestionIndex;

	// ── STAGE 1: CANDIDATE LOGIN ───────────────────────────────────────────────
	if (examStep === "login") {
		return (
			<div
				className='min-h-screen bg-canvas text-text-primary flex items-center justify-center p-4 font-sans select-none relative overflow-hidden'
				data-theme='dark'>
	...
	if (examStep === "instructions" && !session && isStartingSession) {
		return (
			<div
				className='min-h-screen bg-canvas text-text-primary flex items-center justify-center p-4 font-sans'
				data-theme='dark'>
				<div className='text-center space-y-4 max-w-sm'>
					...
					<p className='text-xs text-text-tertiary font-mono'>
						Generating your personalized questions and securing the proctoring
						environment.
					</p>
				</div>
			</div>
				...
				<motion.div
					...
					<motion.div
						...
						<div>
							...
							<p className='font-extrabold text-accent-indigo mt-0.5'>
						...
						<div>
							...
							<p className='font-extrabold text-text-primary mt-0.5'>
						...
						<div>
							...
							<p className='font-extrabold text-text-primary mt-0.5'>
								{rawQuestions.reduce((acc, q) => acc + q.marks, 0)} Marks
							</p>
						</div>
						<div>
							<span className='text-[10px] text-text-tertiary uppercase font-bold'>
							...
							<p className='font-extrabold text-status-success-text mt-0.5'>
								70%
							</p>
						</div>
					</motion.div>
							...
							{sections.map((sec, idx) => (
								<motion.div
									...
									<p className='text-[11px] text-text-tertiary'>
										{sec.questionCount}{" "}
										{sec.questionCount === 1 ? "Question" : "Questions"} •{" "}
					...
					<motion.div
						...
						<p className='text-status-warning-text text-[11.5px] leading-relaxed'>
							• Do not switch tabs, minimize your browser, or exit fullscreen. 3
							warnings will automatically submit your assessment.
							<br />
							• Answers are saved automatically in real-time.
							<br />• Sections are sequential: once you finish a section and
							advance, earlier sections cannot be reopened.
						</p>
					</motion.div>
	...
	if (examStep === "submitted") {
		return (
			<div
				className='min-h-screen bg-canvas text-text-primary flex items-center justify-center p-4 font-sans select-none'
				data-theme='dark'>
					...
					<div className='bg-surface-2 p-4 rounded-2xl border border-border-soft grid grid-cols-2 gap-3 text-xs font-mono text-left'>
						<div>
							...
							<p className='font-bold text-text-primary mt-0.5'>
								{answeredCount} of {rawQuestions.length} ({completionPercentage}
								%)
							</p>
						</div>
						<div>
							...
							<p className='font-bold text-status-success-text mt-0.5'>
								Completed
							</p>
						</div>
					</div>
	...
	if (!currentQuestion) {
		return (
			<div
				className='min-h-screen bg-canvas flex items-center justify-center text-xs text-text-tertiary font-mono'
				data-theme='dark'>
	...
	return (
		<div
			className='h-screen w-screen bg-canvas text-text-primary flex flex-col font-sans select-none overflow-hidden'
			data-theme='dark'>
			{/* ── TOP HUD COMMAND BAR ── */}
			<header className='h-14 bg-surface-1 border-b border-border-default px-4 sm:px-6 flex items-center justify-between shrink-0 z-30'>
				...
				<div className='hidden md:flex items-center gap-4'>
					...
					<div className='flex flex-col items-center'>
						<div className='flex items-center gap-2 text-[10.5px] font-mono text-text-secondary'>
							<span className='text-accent-indigo font-bold'>
								{activeSection?.shortTitle}:
							</span>
							<strong className='text-text-primary'>
								{activeSectionAnsweredCount}/{activeSection?.questionCount}{" "}
								Solved
							</strong>
					...
					<div className='flex flex-col items-center'>
						<div className='flex items-center gap-2 text-[10.5px] font-mono text-text-tertiary'>
							<span>Overall:</span>
							<strong className='text-status-success-text'>
				...
				<div className='flex items-center gap-2 sm:gap-3'>
					...
					<div className='hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-surface-2 border border-border-default text-[10px] font-mono text-text-secondary'>
						<span
							className={`w-2 h-2 rounded-full ${isOnline ? "bg-status-success animate-step-pulse" : "bg-status-warning"}`}
						/>
					...
					<div className='flex items-center gap-2'>
						{sections.length > 1 && activeSection && (
							<div
								className={`px-3 py-1.5 rounded-xl border font-mono text-xs flex items-center gap-1.5 transition-all ${
									(
										(sectionTimeLeftMap[activeSection.sectionId] ??
											activeSection.durationMinutes * 60) < 120
									) ?
						...
						<div
							...
							/>
							<span className='hidden sm:inline'>
								{sections.length === 1 ? "Time Left:" : "Total:"}
							</span>
								...
								<div className='flex flex-col items-center space-y-2 w-full'>
									{sections.map((sec) => {
										...
										const isFutureLocked =
											sec.sectionId > (activeSection?.sectionId || 1);
									...
									<div className='flex flex-col items-center space-y-1.5 w-full'>
										<div className='grid grid-cols-2 gap-1.5 max-h-56 overflow-y-auto px-1 py-1 scrollbar-step'>
											{activeSectionQuestions.map(
												({ q, globalIdx, sectionRelativeIdx }) => {
												...
												<button
													...
													<div className='flex items-center gap-2 min-w-0'>
														...
														<span className='font-bold truncate'>
															Section {sec.sectionId}: {sec.shortTitle}
														</span>
													</div>
									...
									<div className='p-3 rounded-2xl bg-surface-2 border border-border-default space-y-3'>
										<div className='flex items-center justify-between border-b border-border-soft pb-2'>
											...
											<span className='text-[10px] font-mono text-accent-cyan font-bold'>
												...
												{activeSection.questionCount} Solved
											</span>
										</div>
										...
										<div className='grid grid-cols-4 gap-1 text-[9px] font-mono font-bold'>
											<button
												...
												className={`py-1 rounded-lg border text-center transition-all cursor-pointer ${
													...
													:	"bg-surface-3 text-text-tertiary border-border-soft"
												}`}>
											...
											<button
												...
												className={`py-1 rounded-lg border text-center transition-all cursor-pointer ${
													paletteFilter === "ANSWERED" ?
														"bg-status-success text-text-on-accent border-status-success"
													:	"bg-surface-3 text-status-success-text border-border-soft"
												}`}>
											...
											<button
												...
												className={`py-1 rounded-lg border text-center transition-all cursor-pointer ${
													paletteFilter === "UNANSWERED" ?
														"bg-surface-4 text-text-primary border-border-strong"
													:	"bg-surface-3 text-text-tertiary border-border-soft"
												}`}>
												Un (
												{activeSection.questionCount -
													activeSectionAnsweredCount}
												)
											</button>
											<button
												...
												className={`py-1 rounded-lg border text-center transition-all cursor-pointer ${
													paletteFilter === "FLAGGED" ?
														"bg-status-warning text-text-on-accent border-status-warning"
													:	"bg-surface-3 text-status-warning-text border-border-soft"
												}`}>
							...
							<div className='flex items-center justify-between pb-1'>
								<button
									...
									<span className='text-[10px] text-accent-cyan bg-accent-cyan-dim px-2 py-0.5 rounded border border-border-soft'>
										Open Questions ({currentRelativeIndex}/
										{activeSection?.questionCount})
									</span>
								</button>
							</div>
								...
								<motion.div
									...
									<div className='flex items-center gap-2.5 flex-wrap'>
										<span className='px-3 py-1 rounded-xl bg-accent-indigo-dim text-accent-indigo font-mono text-xs font-bold border border-border-default'>
											SECTION {activeSection?.sectionId}:{" "}
											{activeSection?.shortTitle.toUpperCase()}
										</span>
										...
										<span className='text-xs font-mono font-bold text-text-tertiary'>
											QUESTION {currentRelativeIndex} OF{" "}
											{activeSection?.questionCount}
										</span>
									</div>
									...
									<motion.div
										...
										<div className='grid grid-cols-1 gap-2.5'>
											{(currentQuestion.options || []).map((opt, optIdx) => {
												const isSelected = currentAnswer.optionIds.includes(
													opt.id,
												);
												const isMulti =
													currentQuestion.questionType === "MULTI_CHOICE";
									...
									<motion.div
										...
										<div className='rounded-2xl border border-border-default bg-surface-2 overflow-hidden'>
											<CodeEditorIDE
												...
												onChange={(val) =>
													handleTextAnswerChange(currentQuestion.id, val)
												}
												language={
													currentQuestion.programmingLanguage || "csharp"
												}
												questionType='CODING'
												defaultTemplate={
													currentQuestion.codeTemplate ||
													"// Write your C# / TypeScript solution here..."
												}
												title='CODE SOLUTION'
											/>
											...
											<div className='border-t border-border-default bg-surface-1 p-3.5 flex items-center justify-between flex-wrap gap-2'>
												<div className='flex items-center gap-2 text-xs font-mono text-text-tertiary'>
													...
													/>
													<span>
														Write and structure your code solution. It will be
														reviewed by the technical evaluator.
													</span>
												</div>
												...
												<div className='flex items-center gap-2'>
													...
													<motion.button
														...
														whileTap={{ scale: 0.97 }}
														onClick={() =>
															handleExplicitSave(currentQuestion.id)
														}
									...
									<motion.div
										...
										<div className='flex items-center justify-between bg-surface-2 p-3 rounded-2xl border border-border-default'>
											<span className='text-xs font-mono text-accent-cyan font-bold flex items-center gap-2'>
												...
												/>
												<span>
													Schema: Employees (Id INT, FullName VARCHAR, Salary
													DECIMAL, DepartmentId INT)
												</span>
											</span>
										...
										<div className='rounded-2xl border border-border-default bg-surface-2 overflow-hidden'>
											...
											<div className='border-t border-border-default bg-surface-1 p-3.5 flex items-center justify-between flex-wrap gap-2'>
												<div className='flex items-center gap-2 text-xs font-mono text-text-tertiary'>
													...
													/>
													<span>
														Compose your SQL query. Target Engine: T-SQL / SQL
														Server 2022.
													</span>
												</div>
									...
									<motion.div
										...
										<div className='flex items-center justify-between text-xs font-mono font-bold text-text-tertiary'>
											...
											<span className='text-accent-indigo'>
												...
												/ {currentQuestion.maxWordCount || 500} Words
											</span>
										</div>
							...
							<motion.button
								...
								title={
									isFirstQuestionOfSection ?
										"Cannot navigate back to previously locked section"
						...
						<motion.div
							...
							<div className='space-y-2'>
								...
								<p className='text-xs text-text-secondary'>
									Time for{" "}
									<strong>
										{sectionTransitionModal.completedSectionTitle}
									</strong>{" "}
									has expired. The section is now locked.
								</p>
							</div>
							...
							<div className='p-4 rounded-2xl bg-surface-2 border border-border-default space-y-3 text-left'>
								<div className='flex items-center justify-between text-xs font-mono'>
									<span className='text-text-tertiary uppercase font-bold'>
										Next Round
									</span>
								...
								<div className='flex items-center gap-4 text-xs font-mono text-text-tertiary'>
									<span>
										...
										<strong className='text-text-secondary'>
											{sectionTransitionModal.nextSectionQuestionCount}
										</strong>
									</span>
									<span>
										Duration:{" "}
										<strong className='text-text-secondary'>
											{sectionTransitionModal.nextSectionDurationMinutes} mins
										</strong>
									</span>
								</div>
							</div>
							...
							<div className='space-y-2'>
								<div className='flex items-center justify-between text-xs font-mono text-text-secondary'>
									...
									<strong className='text-accent-cyan font-bold font-mono'>
										00:
										{sectionTransitionModal.countdownSeconds
											.toString()
											.padStart(2, "0")}
									</strong>
								</div>
								<div className='w-full h-1.5 bg-surface-3 rounded-full overflow-hidden'>
									<motion.div
										...
										initial={false}
										animate={{
											width: `${(sectionTransitionModal.countdownSeconds / 30) * 100}%`,
										}}
							...
							<div className='pt-1'>
								<motion.button
									...
									<span>
										Start {sectionTransitionModal.nextSectionTitle} Immediately
									</span>
									<Icon
										name='arrow-right'
										size='xs'
			...
			{fullscreenReentrySeconds !== null &&
				...
				examStep === "active" && (
					<div
						...
						className='fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 cursor-pointer select-none'>
						<div
							onClick={(e) => e.stopPropagation()}
							className='bg-[var(--surface-1)] border border-rose-500/40 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl text-center flex flex-col items-center gap-4 animate-in fade-in zoom-in-95'>
							...
							<div className='flex flex-col gap-1.5'>
								...
								<p className='text-xs text-[var(--text-secondary)] leading-relaxed'>
									This proctored assessment requires full-screen mode to ensure
									test integrity. Click anywhere or press the button below to
									resume full screen.
								</p>
							</div>
							...
							<button
								type='button'
								onClick={requestAppFullscreen}
								className='w-full h-11 rounded-xl bg-[var(--accent-indigo)] hover:bg-[var(--accent-indigo)]/90 text-white font-extrabold text-xs transition-all shadow-md cursor-pointer flex items-center justify-center gap-2 mt-1 animate-bounce'>
							...
							<p className='text-[10px] text-[var(--text-tertiary)]'>
								Tip: Pressing{" "}
								<kbd className='px-1 py-0.5 rounded bg-[var(--surface-3)] font-mono text-[9px]'>
									Enter
								</kbd>{" "}
								or clicking anywhere re-enters full screen.
							</p>
						</div>
					</div>
				)}
			...
			<ExamSubmissionModal
				...
				onConfirm={() =>
					handleFinalSubmit("Candidate confirmed assessment submission")
				}
