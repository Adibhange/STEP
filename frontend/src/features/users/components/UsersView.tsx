...
export const UsersView: React.FC = () => {
	...
	const { data: deptMasterRes } = useGetMasterDataByCategoryQuery(
		"departments",
		{
			skip: isInterviewer,
		},
	...
	const displayUsers: UserItem[] = (apiUsersResponse?.data || []).map(
		(u: any) => ({
			id: String(u.id),
	...
	const handleOpenEdit = (user: UserItem) => {
		...
		setFormDept(
			user.department === "—" ? departmentOptions[0] || "IT" : user.department,
		);
		...
		return (
			<div className='flex flex-col items-center justify-center py-24 px-4 text-center max-w-md mx-auto'>
				...
				<p className='text-[13px] text-[var(--text-secondary)]'>
					The Users & Access Control directory is restricted to Directors and
					HRs. Interviewers have access to assigned candidate workspaces and
				...
				<div className='flex flex-col gap-3.5'>
					...
					<div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
						...
						<div>
							...
							<CustomSelect
								label='Department'
								value={formDept}
								options={departmentOptions.map((d: string) => ({
									value: d,
									label: d,
								}))}
								onChange={(val) =>
									setFormDept(val || departmentOptions[0] || "IT")
								}
								widthClass='w-full'
							/>
						</div>
					...
					<div>
						...
						<CustomSelect
							...
							onChange={(val) => setFormStatus((val as UserStatus) || "Active")}
							widthClass='w-full'
						/>
					</div>
				</div>
					...
					<div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
						<div>
							...
							<CustomSelect
								...
								options={USER_ROLE_OPTIONS}
								onChange={(val) =>
									setFormRole((val || "Interviewer") as UserRole)
								}
								widthClass='w-full'
							/>
						</div>
