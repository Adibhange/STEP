...
export default function CandidateRegistrationPortalPage() {
	...
	const [registerCandidate, { isLoading: isRegistering }] =
		useRegisterCandidateViaQRMutation();
		...
		switch (name) {
			...
			case "cgpaOrPercentage":
				...
				if (value.trim()) {
					...
					if (isNaN(num) || num <= 0 || num > 100)
						return "Enter a valid score (e.g. 8.4 or 80).";
				}
				return "";
			case "experienceYears":
				if (candidateType === "Experienced") {
					...
					if (isNaN(num) || num <= 0 || num > 40)
						return "Enter experience between 1 and 40 years.";
				}
				return "";
			case "currentCompany":
			...
			case "refName":
				if (refType !== "Direct" && !value.trim())
					return "Referrer / Contact name is required.";
				return "";
			case "refMobile":
				if (refType !== "Direct" && value.trim()) {
	...
	const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (!file.type.startsWith("image/")) {
			setFileErrors((prev) => ({
				...prev,
				photo: "Please upload a valid image file (JPG, PNG, WEBP).",
			}));
			return;
		}
		...
		if (file.size > 2 * 1024 * 1024) {
			setFileErrors((prev) => ({
				...prev,
				photo: "Photo size exceeds the 2MB limit.",
			}));
			return;
		}
	...
	const handleResumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (!validExtensions.includes(fileExt)) {
			setFileErrors((prev) => ({
				...prev,
				resume: "Please upload a PDF or Word document (.pdf, .docx).",
			}));
			return;
		}
		...
		if (file.size > 5 * 1024 * 1024) {
			setFileErrors((prev) => ({
				...prev,
				resume: "Resume file size exceeds the 5MB limit.",
			}));
			return;
		}
		...
		try {
			const payload = {
				...
				totalExperienceYears:
					candidateType === "Fresher" ? 0 : Number(experienceYears) || 1,
				currentLocation: currentLocation.trim(),
				highestQualification: qualification,
				institutionName: collegeName.trim() || undefined,
				yearOfPassing: Number(passingYear) || undefined,
				marksPercentage:
					cgpaOrPercentage ? Number(cgpaOrPercentage) : undefined,
				currentCompany:
					candidateType === "Experienced" ? currentCompany.trim() : undefined,
				currentDesignation:
					candidateType === "Experienced" ? currentCompany.trim() : undefined,
				currentCTC:
					candidateType === "Experienced" && currentCtc ?
						Number(currentCtc)
					:	undefined,
				expectedCTC:
					candidateType === "Experienced" && expectedCtc ?
						Number(expectedCtc)
					:	undefined,
				noticePeriodDays:
					candidateType === "Experienced" ? Number(noticePeriod) : 0,
				refType,
				refName: refType !== "Direct" ? refName.trim() : undefined,
				refMobile:
					refType !== "Direct" && refMobile ?
						refMobile.trim().replace(/[\s+-]/g, "")
					:	undefined,
											...
											<motion.div
												...
												:	<div className='text-center p-1'>
														<Icon
															name='user'
															size='xs'
															className='text-[var(--text-tertiary)] mx-auto group-hover:text-[var(--accent-indigo)] transition-colors'
														/>
														<span className='text-[9px] font-bold text-[var(--text-tertiary)] block mt-0.5'>
															Photo
														</span>
													</div>
												}
										...
										<div className='flex items-center p-1 rounded-xl bg-[var(--surface-1)] border border-[var(--border-default)] shrink-0 self-start sm:self-center relative'>
											<button
												...
												className={`relative z-10 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer ${
													candidateType === "Fresher" ? "text-white" : (
											...
											<button
												...
												className={`relative z-10 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer ${
													candidateType === "Experienced" ? "text-white" : (
														"text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
													)
												}`}>
												{candidateType === "Experienced" && (
										...
										<div className='space-y-1'>
											...
											<input
												...
												onChange={(e) => {
													...
													if (touched.currentLocation)
														setErrors((prev) => ({
															...prev,
															currentLocation: validateField(
																"currentLocation",
																e.target.value,
															),
														}));
												...
												onBlur={() =>
													handleBlur("currentLocation", currentLocation)
												}
												className={`w-full h-10 px-3.5 rounded-xl border bg-[var(--surface-2)] text-xs text-[var(--text-primary)] focus:outline-none transition-all ${
									...
									<div className='grid grid-cols-1 sm:grid-cols-2 gap-3.5'>
										...
										<div className='space-y-1'>
											<label className='text-xs font-bold text-[var(--text-secondary)]'>
												...
												{candidateType === "Fresher" ? "*" : "(Optional)"}
											</label>
											<input
												type='number'
												step='any'
												...
												onChange={(e) => {
													...
													if (touched.cgpaOrPercentage)
														setErrors((prev) => ({
															...prev,
															cgpaOrPercentage: validateField(
																"cgpaOrPercentage",
																e.target.value,
															),
														}));
											...
											<motion.div
												...
												<div className='space-y-1'>
													...
													<input
														type='text'
														placeholder={
															refType === "Internal" ? "e.g. Rahul Sharma" : (
																"e.g. ABC Staffing / Amit Verma"
															)
														}
														value={refName}
														onChange={(e) => {
												...
												<div className='space-y-1'>
													...
													<input
														...
														onChange={(e) => {
															...
															setRefMobile(clean);
															if (touched.refMobile)
																setErrors((prev) => ({
																	...prev,
																	refMobile: validateField("refMobile", clean),
																}));
														}}
												...
												<div className='space-y-1'>
													...
													<input
														...
														onChange={(e) => {
															...
															if (touched.currentCompany)
																setErrors((prev) => ({
																	...prev,
																	currentCompany: validateField(
																		"currentCompany",
																		e.target.value,
																	),
																}));
								...
								<div className='space-y-1.5'>
									...
									<p className='text-xs text-[var(--text-secondary)] max-w-md mx-auto leading-relaxed'>
										...
										<span className='font-bold text-[var(--text-primary)]'>
											{registeredResult.firstName} {registeredResult.lastName}
										</span>
								...
								<motion.div
									...
									<div className='space-y-1 text-xs'>
										...
										<p className='text-[11.5px] text-[var(--text-secondary)] leading-relaxed'>
											Click the button below to start your online aptitude test
											immediately. Upon completion, the HR team will review your
											score.
										</p>
									</div>
								</motion.div>
